...
type Users struct {
	...
	Openid        string    `json:"openid" orm:"column(openid);size(255)"`
	Subscribe     int       `orm:"column(subscribe)"`
	Nickname      string    `json:"nickname" orm:"column(nickname);size(255);null"`
	Sex           int       `json:"sex" orm:"column(sex);null"`
	City          string    `json:"city" orm:"column(city);size(255);null"`
	Country       string    `json:"country" orm:"column(country);size(255);null"`
	Province      string    `orm:"column(province);size(255);null"`
	Language      string    `orm:"column(language);size(255);null"`
	Headimgurl    string    `json:"headimgurl" orm:"column(headimgurl);size(255);null"`
	SubscribeTime time.Time `orm:"column(subscribe_time);type(timestamp);null"`
	Unionid       string    `json:"unionid" orm:"column(unionid);size(255);null"`
	Createdtime   string    `orm:"column(createdtime);size(255)"`
	Updatetime    time.Time `orm:"column(updatetime);type(timestamp)"`
}
...
// GetUsersById retrieves Users by Id. Returns error if
// Id doesn't exist
func GetUsersByOpenId(openid string) (v *Users, err error) {
	o := orm.NewOrm()
	v = &Users{Openid: openid}
